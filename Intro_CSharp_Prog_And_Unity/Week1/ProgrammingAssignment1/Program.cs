    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (input[0] != 'q')
            {
                ...
                float distanceXY = (float) Math.Sqrt((double)((deltaX * deltaX) + (deltaY * deltaY)));

                double angleInRadian = Math.Atan2((double) deltaY, (double) deltaX);
                double angleInDegree = angleInRadian * (180d / Math.PI);
                Console.WriteLine(distanceXY + " " + angleInDegree);
